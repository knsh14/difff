...
export class GitService {
    ...
    async getAllRefs(): Promise<string[]> {
        try {
            ...
            try {
                ...
                Object.keys(branches.branches).forEach(branch => {
                    if (branch.startsWith('remotes/')) {
                        const cleanBranch = branch.replace('remotes/origin/', '').replace('remotes/', '');
    ...
    async getDiff(baseRef: string, compareRef: string): Promise<DiffResult> {
        try {
            ...
            const diff = await this.git.diff([`${base}...${compare}`, '--name-status']);
    ...
    async getFileDiff(baseRef: string, compareRef: string, filePath: string): Promise<string> {
        try {
            const base = this.parseRef(baseRef);
            const compare = this.parseRef(compareRef);
    ...
    async getDiffFiles(baseRef: string, compareRef: string): Promise<Array<{path: string, status: string, additions: number, deletions: number}>> {
        try {
            const base = this.parseRef(baseRef);
            const compare = this.parseRef(compareRef);
